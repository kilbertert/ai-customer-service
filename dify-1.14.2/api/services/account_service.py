...
class AccountService:
    ...
    def create_account(
        ...
        db.session.add(account)
        # basjoo M11 PR1: flush instead of commit so caller can wrap in single transaction
        db.session.flush()
        return account

    @staticmethod
    def create_account_with_password(email: str, name: str, password: str) -> Account:
        """basjoo M11 PR1: 暴露外部调用的 account 创建,接受明文密码(Dify 内部 hash).

        与 create_account 的关键差异:
        - 不依赖 is_setup,直接走 valid_password 校验
        - 内部 flush 而非 commit,允许调用方包裹在单事务内
        - 返回的 Account 对象已带 id (flush 后 SQLAlchemy 赋值)
        """
        valid_password(password)
        salt = secrets.token_bytes(16)
        base64_salt = base64.b64encode(salt).decode()
...
class TenantService:
    ...
    def create_tenant(name: str, is_setup: bool | None = False, is_from_dashboard: bool | None = False) -> Tenant:
        ...
        # basjoo M11 PR1: flush instead of commit
        db.session.flush()
    ...
    def provision_tenant_by_admin(
        ...
        """basjoo M11 PR1: admin 调用,一次性创建 tenant + owner account + 绑定关系.

        与 create_owner_tenant_if_not_exist 的关键差异:
        - 不依赖当前登录 user(account 参数是 owner_email/name)
        - bypass is_allow_create_workspace 校验(直接走 provisioning path)
        ...
        if existing:
            ...
            if owner_join:
                return {
                    ...
                    "initial_password": existing.initial_password_plain or _generate_initial_password(),
                    "status": "ready",
                    "idempotent_replay": True,
                }

        # 2. 创建 tenant(bypass is_allow_create_workspace,直接构造)
        now = naive_utc_now()
        tenant = Tenant(
            name=name,
            custom_idempotency_key=idempotency_key,
            created_via_admin_at=now,
        )
        db.session.add(tenant)
        try:
            db.session.flush()
            ...
            db.session.rollback()
            raise TenantProvisionConflictError(f"Failed to create tenant: {e}")

        # 3. 创建 owner account(走 create_account_with_password)
        try:
            ...
            db.session.rollback()
            raise AdminProvisionForbiddenError(f"Password complexity failed: {e}")
        except Exception as e:
            db.session.rollback()
            raise AdminProvisionForbiddenError(f"Failed to create owner account: {e}")

        # 4. 暂存明文密码(24h TTL 由 GET owner-credentials endpoint 配合读取)
        tenant.initial_password_plain = owner_password

        # 5. 创建 TenantAccountJoin(role='owner')
        try:
            TenantService.create_tenant_member(tenant, account, role="owner")
        except Exception as e:
            db.session.rollback()
        ...
        # 6. 触发 tenant_was_created signal(Dify 默认行为)
        tenant_was_created.send(tenant)
...
def _generate_initial_password(length: int = 32) -> str:
    """basjoo M11 PR1: 生成 32 字节随机密码,Dify 不持久化,仅返回给 basjoo."""
