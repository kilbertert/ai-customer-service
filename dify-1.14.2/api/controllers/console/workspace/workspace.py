...
from controllers.console.workspace.models import TenantProvisionPayload, TenantProvisionResponse
...
from models.account import Tenant, TenantAccountJoin, TenantAccountRole, TenantCustomConfigDict, TenantStatus
from services.account_service import TenantService
from services.billing_service import BillingService, SubscriptionPlan
from services.enterprise.enterprise_service import EnterpriseService
from services.errors.account import AdminProvisionForbiddenError, TenantProvisionConflictError
...
# basjoo M11 PR1: admin provision response fields
tenant_provision_response_fields = {
    "workspace_id": fields.String,
    "owner_account_id": fields.String,
    "initial_password": fields.String,
    "status": fields.String,
    "idempotent_replay": fields.Boolean,
}
...
# basjoo M11 PR1: admin-only tenant provisioning endpoints
...
class TenantProvisionOwnerCredentialsApi(Resource):
    ...
    def get(self, tenant_id):
        ...
        if not tenant:
            return {"error": "not_found", "message": "Tenant not found"}, 404

        # 仅 24h 内由 admin 创建的 tenant 才返回明文密码
        if not tenant.created_via_admin_at or tenant.initial_password_plain is None:
            return {"error": "conflict", "message": "Credentials no longer retrievable"}, 409
        ...
        if not owner_join:
            return {"error": "not_found", "message": "Owner not found"}, 404

        return {
            "owner_account_id": owner_join.account_id,
            "initial_password": tenant.initial_password_plain,
        }, 200
...
class TenantProvisionRollbackApi(Resource):
    ...
    def delete(self, tenant_id):
        ...
        # 删除顺序:join → tenant (cascade 清 account 由 DB 约束保证)
        joins = list(
            db.session.scalars(
